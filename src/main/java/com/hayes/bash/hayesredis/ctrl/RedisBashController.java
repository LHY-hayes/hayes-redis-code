...
@RequestMapping("redis_base_controller")
public class RedisBashController {
    ...
    public JsonResult sput(@RequestBody JSONObject jsonObject){
        ...
        if (!jsonObject.containsKey("key")) return JsonResult.failure(ResultCode.JSON_NO_PARAM_KEY);
        if (!jsonObject.containsKey("phoneNumbers")) return JsonResult.failure(ResultCode.JSON_NO_PARAM_PHONENUMBERS);
